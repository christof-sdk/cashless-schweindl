...
- [x] **1.13 Natives `confirm()`/`prompt()`/`alert()` bricht mit dem gestalteten Look.** → Erledigt: neue generische Bottom-Sheet-Dialog-Komponente (`showAppDialog()`/`appConfirm()`/`appPrompt()`/`appAlert()`, gleiche visuelle Sprache wie das „Sparziel erreicht"-Sheet) ersetzt alle 5 Stellen (Owner-Claim-Confirm+Prompt, Passwort-Prompt, Karte-vergessen-Confirm, Reset-Confirm mit Danger-Stil, Pending-Tap-Info). Passwort-Eingabe ist jetzt maskiert (`type="password"`), was der native `prompt()` nie konnte. In beiden Farbschemata visuell geprüft; dabei einen Race-Condition-Bug beim schnellen Hintereinander-Öffnen zweier Dialoge gefunden und gefixt.
