...
- **Storage: per-jar, not per-device.** The scheme is `jars/{id}.colorScheme` (`'1'` or `'2'`, validated in `database.rules.json`), toggled via two "Schema 1"/"Schema 2" buttons in the **Einstellungen** sheet. This was a deliberate choice over a `localStorage`-only per-device preference: the color scheme is a branding decision for that specific piggy bank — like its name — and everyone who opens that jar's link should see the same thing, not a different look depending on who's currently viewing it.
