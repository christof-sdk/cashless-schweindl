...
- Gated actions: rename/change goal (`saveSettings`), "Schweinchen zurücksetzen" (`resetJar`), payout threshold + "Jetzt abrechnen" (`saveThreshold`/`chargeNow`). **Not** gated: tapping to pay, viewing the dashboard, managing the device's own card ("Karte vergessen"/"hinterlegen" — that's device-scoped, not jar-scoped).
